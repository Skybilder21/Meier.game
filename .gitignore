...
function rollDice() {
  ...
  if (free.length === 0) { alert('Alle Würfel sind beiseitegelegt'); return; }
  rollsThisTurn++;
  free.forEach(d=>{
...
function calcBonus() {
  ...
  let b = 0;
  for (const v in counts) if (counts[v] >= 2) b += counts[v];
  return b;
}
...
document.addEventListener('keydown', (e) => {
  if (!players[curPlayer]) return;
  switch(e.key.toLowerCase()) {
    case 'm': // Meier Cheat
      dice.forEach((d,i)=>d.value = i+1); // 1-6
      renderDice();
      updateInfo();
      log(`${players[curPlayer].name} benutzte Cheat: MEIER!`);
      break;
    case 'c': // +50 Punkte
      players[curPlayer].score += 50;
      renderPlayers();
      log(`${players[curPlayer].name} benutzte Cheat: +50 Punkte`);
      break;
    case 'h': // Alle Würfel festhalten
      dice.forEach(d => d.held = true);
      renderDice();
      updateInfo();
      log(`${players[curPlayer].name} benutzte Cheat: Alle Würfel festgehalten`);
      break;
    case 'r': // Mega-Runde: alle 6
      dice.forEach(d => { d.value = 6; d.held = false; });
      renderDice();
      updateInfo();
      log(`${players[curPlayer].name} benutzte Cheat: Mega-Runde (alle 6)`);
      break;
  }
});
